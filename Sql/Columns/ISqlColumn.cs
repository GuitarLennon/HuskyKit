...
namespace HuskyKit.Sql.Columns
{
    ...
    public abstract class ISqlColumn : ISqlExpression
    {
        ...
        public virtual string GetOrderByExpression(BuildContext context, int targetIndex)
        {
            var a = "[{" + targetIndex.ToString() + "}]" + $".[{Name}]";
            return string.Format(a, context.TableAlias);
        }
